...
const returnList = () => {
  return Promise.resolve(["eggs", "apples", "milk", "bread"]);
};
...
const amIYourFather = (string) => {
  if (string == "Luke") {
    return Promise.resolve("Yes. Luke, I am your father.");
  } else {
    return Promise.reject("Not your dad.");
  }
};
...
const myNameIs = () => {
  return new Promise((myResolve) =>
    myResolve((value) => `My name is ${value}`)
  );
};
