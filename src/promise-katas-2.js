...
const food = () => {
  return fetch("food").then((resolve) => resolve.data);
};
...
const cat = () => {
  return fetch("cats").then((resolve) => resolve.data.cats.sort());
};
...
// GO BACK AND DO THIS ONE
const dog = () => {
  return fetch("dogs").then(() => {
    return { name: "Mutley", naughty: 10 };
  });
};
